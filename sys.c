...
SYSCALL_DEFINE2(set_sched_hint, pid_t, pid, int, hint)
{
    struct task_struct *task;
    const struct cred *cred = current_cred(), *tcred;
    int ret = 0;

    /* 1. Validate the hint value */
    if (hint < HINT_NONE || hint > HINT_LATENCY_SENSITIVE) {
        return -EINVAL;
    }

    /* RCU read lock is required when looking up tasks to ensure 
       the task doesn't get destroyed while we are examining it */
    rcu_read_lock();

    /* 2. Find the target task */
    if (pid == 0) {
        task = current; /* A PID of 0 means the calling process */
    } else {
        task = find_task_by_vpid(pid);
    }

    /* Check if the PID actually exists */
    if (!task) {
        rcu_read_unlock();
        return -ESRCH;
    }
    ...
    /* The caller can modify the hint if:
       - They are modifying their own process (or a process owned by the same user)
       - OR they have CAP_SYS_NICE (root/admin privileges for scheduling) */
    if (!uid_eq(cred->euid, tcred->euid) && 
